...
function CpfConsulta() {
  ...
  const fetchData = async () => {
    ...
    try {
      const cpfResponse = await axios.get(`http://65.21.240.99:5000/api/consulta_cpf?cpf=${cpf}`);
      setCpfData(cpfResponse.data.complete_data);

      const dadosGeraisResponse = await axios.get(`http://65.21.240.99:5000/consulta/${cpf}`);
